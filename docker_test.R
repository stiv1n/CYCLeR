...
###################################################
sgfc_pred <- analyzeFeatures(sc, which=BSJ_gr , min_junction_count=2, beta =0.1 , min_n_sample=1,cores=1,verbose=F)
...
write.table(qics_out_final[,-9],file = "/usr/workdir/dm_circles.txt", sep = "\t",row.names = F, col.names = T,quote=F)
